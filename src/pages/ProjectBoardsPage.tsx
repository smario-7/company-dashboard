  ...
  const handleArchive = async (board: Board) => {
    ...
    try {
      ...
      if (!showArchived) {
        ...
        setBoards(prev => prev.map(b =>
          b.slug === board.slug ? { ...b, archived: !b.archived } : b
        ))
      }
    } finally { setArchiving(null); setMenuOpen(null) }
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 animate-fade-in">
      ...
      <div className="border-b border-white/5 px-6 py-4">
        <Link to="/projects"
          className="flex items-center gap-1 text-xs text-surface-200/40 hover:text-surface-200 mb-3 w-fit transition-colors">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl text-2xl flex-shrink-0"
    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 mb-3">
        {board.columns.slice(0, 4).map(col => (
          <span key={col.id} className="badge text-[10px]"
