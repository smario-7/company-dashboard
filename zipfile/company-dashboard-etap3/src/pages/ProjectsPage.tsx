...
export function ProjectsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      setProjects(await svc.listProjects(showArchived))
    } finally {
      setLoading(false)
    }
  }, [storage, showArchived]) // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { load() }, [load])

  const handleCreate = async (data: Pick<Project, 'name' | 'description' | 'color' | 'emoji'>) => {
    if (!svc || !user) return
    await svc.createProject(data, user.githubLogin)
    await load()
  }
    ...
    <div className="flex flex-col flex-1 min-h-0 animate-fade-in">
      ...
      <div className="flex items-center justify-between border-b border-white/5 px-6 py-4">
        <div>
          ...
          <p className="text-xs text-surface-200/40 mt-0.5">{projects.length} project{projects.length !== 1 ? 's' : ''}</p>
        </div>
...
function EmptyState({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 gap-4">
      <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-surface-800 ring-1 ring-white/5 text-3xl">
        📋
      </div>
