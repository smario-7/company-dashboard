...
export class GitHubStorage {
  ...
  /** Remove a collaborator from the repository (used when deleting a user). */
  ...
  /** List all repository collaborators (login + avatar). */
  async listCollaborators(): Promise<Array<{ login: string; avatar_url: string }>> {
    ...
    return data.map(c => ({ login: c.login, avatar_url: c.avatar_url }))
  }
